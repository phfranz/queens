...
class Gaussian(Likelihood):
    ...
    def __init__(
        ...
        Args:
            ...
            y_obs (array_like): observations
                - vector for univariate observations
                - matrix for multivariate observations, where columns represent the
                  different observations and rows the "features" per observation
        ...
        # additional attribute to avoid too many changes
        self.cov = covariance
    ...
    def _evaluate(self, samples):
        ...
        for n in range(len(samples)): 
            ...
            #log_likelihood[n] += Normal(self.response["result"][n,:], self.cov).logpdf(self.y_obs.T) #Transpose is required here, 

            # such that number of variable in multivariate normal distribution fits the number of features per sample.

            log_likelihood[n] += sp.stats.multivariate_normal.logpdf(self.y_obs.T, mean=self.response["result"][n,:], cov=self.cov).sum(axis=0)
